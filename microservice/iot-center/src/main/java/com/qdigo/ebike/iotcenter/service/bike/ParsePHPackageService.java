...
import com.qdigo.ebike.iotcenter.message.bike.PHManage;
...
public class ParsePHPackageService extends ParsePackageServiceImpl {
    ...
    public PHPacketDto parseUpBytes(byte[] bytes, char header0, char header1, int imei, String client) {
        logger.debug("PH system:" + DateUtil.format(new Date(), DateUtil.YMDHMS_PATTERN));
        byte seq = bytes[6];
        byte status = bytes[7];
        ...
        PHPacketDto phPacketDto = buildPHPacketDto(header0, header1, imei, client, seq, status, imsi,
    ...
    private PHPacketDto buildPHPacketDto(char header0, char header1, int imei, String client,
        ...
        logger.debug("phPacketDto[" + phPacketDto + "]");
        return phPacketDto;
    }
}
