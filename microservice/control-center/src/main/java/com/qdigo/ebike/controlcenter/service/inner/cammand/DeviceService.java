...
public class DeviceService {
    ...
    @Inject
    private PHMongoService phMongoService;
    @Inject
    ...
    private void insertPX(final byte[] bytes, final String mobileNo) {
        if(env.getProperty("my.env").equals("test")) return;
        ...
        PXPackage px = PXPackage.builder().pxImei(imei).pxCmd(cmd).pxParam(param)
            .pxSequence(seq).timestamp(System.currentTimeMillis()).mobileNo(mobileNo).build();
    ...
    public boolean ignition(String imei, String mobileNo) {
        if(env.getProperty("my.env").equals("test")) return true;
    ...
    public boolean lock(String imei, String mobileNo, int count) {
        if(env.getProperty("my.env").equals("test")) return true;
        Transaction t = Cat.newTransaction("method", "lock");
        try {
            boolean shutdown = DeviceCtrl.Lock(imei, bytes -> insertPX(bytes, mobileNo), count);
            t.setSuccessStatus();
            return shutdown;
        } catch (Exception e) {
            t.setStatus(e);
    ...
    public boolean shutdown(String imei, String mobileNo, int count) {
        if(env.getProperty("my.env").equals("test")) return true;
        Transaction t = Cat.newTransaction("method", "shutdown");
        try {
            boolean shutdown = DeviceCtrl.Shutdown(imei, bytes -> insertPX(bytes, mobileNo), count);
            t.setSuccessStatus();
            return shutdown;
        } catch (Exception e) {
            t.setStatus(e);
